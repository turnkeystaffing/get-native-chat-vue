...
The library is backend-agnostic: consumers provide an API client implementation (or use the included Axios-based default) that handles conversation and message CRUD operations.
...
| Testing | Vitest 4 (300+ unit/integration tests) + Playwright (perf benchmarks) |
| Docs | VitePress ^1.6 |
| Package Manager | Yarn 4 (Berry) |
| Runtime Deps | marked (markdown), DOMPurify (XSS sanitization) |
| Optional Peer | Axios ^1.0 (for default API client helper) |
...
- **API abstraction** via `NativeChatApiClient` interface — any backend, default Axios adapter included
...
├── src/                   Library source (8 components, 7 icons, 1 composable, 1 helper, types)
...
- **Helper Export:** `createNativeChatApiClient` (Axios adapter API client)
...
- **Test Command:** `yarn test` (300+ tests)
...
- [Source Tree Analysis](./source-tree-analysis.md) — Annotated directory tree, data flow diagrams, file statistics
- [Component Inventory](./component-inventory.md) — All components with props, behaviors, Vuetify dependencies, accessibility
- [Development Guide](./development-guide.md) — Setup, scripts, testing, linting, publishing, common tasks
